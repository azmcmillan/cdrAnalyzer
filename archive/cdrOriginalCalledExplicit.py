...
while(continueLookup):
    ...
    csvData['originalCalledPartyNumber'] = csvData['originalCalledPartyNumber'].apply(str)

    for num in callNum:
        num = str(num)
        # Search through CSV data for called party number.
        result = csvData[csvData['originalCalledPartyNumber']==num]
        #result = csvData[csvData['originalCalledPartyNumber'].str.contains(num, regex=True)]

        # Strip all columns except the following
        result = result[['dateTimeOrigination', 'callingPartyNumber', 'originalCalledPartyNumber', 'globalCallID_callId', 'duration']]
        ...
        if uInput:
            ...
            print('dateTime, callId, duration, calling, originalCalled')

            for call in result.index:
                calling = result['callingPartyNumber'][call]
                called = result['originalCalledPartyNumber'][call]
